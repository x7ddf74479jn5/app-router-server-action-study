...
export default async function Chats({ chats }: { chats: Chat[] }) {
  ...
  return (
    <div className="h-96 overflow-auto text-left">
      {chatList.map((chat) => {
        return (
          <div key={chat.id} className={`flex items-start p-2 md:p-3 ${chat.isMe ? "flex-row-reverse" : ""}`}>
            <div className="flex flex-col items-center">
              ...
              <p className="text-center text-xs text-gray-500 md:text-sm">{chat.name ?? "ゲスト"}</p>
            </div>
            <p className="mx-1 justify-self-stretch p-4 text-xs md:mx-4 md:text-sm">{chat.message}</p>
          </div>
        );
      })}
    </div>
